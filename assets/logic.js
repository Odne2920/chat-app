...
export function initHRNchat(
    customConfig = {})
{
    const CONFIG = {
        supabaseUrl: customConfig
            .supabaseUrl ||
            "https://jnhsuniduzvhkpexorqk.supabase.co",
        supabaseKey: customConfig
            .supabaseKey ||
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImpuaHN1bmlkdXp2aGtwZXhvcnFrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE1NjAxMDYsImV4cCI6MjA4NzEzNjEwNn0.9I5bbqskCgksUaNWYlFFo0-6Odht28pOMdxTGZECahY",
        mailApi: customConfig
            .mailApi ||
            "https://vercel-serverless-hrn.vercel.app/api/mailAPI",
        maxUsers: customConfig
            .maxUsers || 150,
        maxMessages: customConfig
            .maxMessages || 50,
        historyLoadLimit: customConfig
            .historyLoadLimit || 20,
        rateLimitMs: customConfig
            .rateLimitMs || 1000,
        presenceHeartbeatMs: customConfig
            .presenceHeartbeatMs ||
            10000,
        verificationCodeExpiry: customConfig
            .verificationCodeExpiry ||
            600,
        maxMessageLength: customConfig
            .maxMessageLength ||
            10000,
        proxyUrl: customConfig
            .proxyUrl ||
            "https://vercel-serverless-hrn.vercel.app/api/CORSproxy.js?url="
    };
    const AVATARS = [
    ...
    const setAppMode = (offline) =>
    {
        ...
        setConnectionVisuals(
            offline ?
            'offline' :
            'connected');
    };
    const setConnectionVisuals = (
        status) =>
    {
        const avatar = $(
            'chat-avatar-display'
    ...
    const updatePresenceUI = () =>
    {
        ...
        else
        {
            ...
            let displayGlobalCount =
                `${state.globalOnlineCount}/${CONFIG.maxUsers}`;
            if (infoGlobalEl)
                infoGlobalEl
                .innerText =
                displayGlobalCount;
        ...
        null) =>
    {
        ...
        const loaderText = $(
            'loader-text');
        if (!loader) return;
        if (s) loader.classList.add(
            'active');
        else loader.classList
            .remove('active');
        if (text) loaderText
            .innerText = text;
        else loaderText.innerText =
            "Loading...";
    };
        ...
        userId) =>
    {
            ...
            .isOfflineMode)
        {
            ...
            {
                console.warn(
                    "Profile fetch failed, using cache if available",
                    e);
            }
        }
        if (profile)
    ...
    const resolveRoomDisplay = async (
        room) =>
    {
        if (!room) return {
            name: 'Chat',
    ...
    const cryptoWorker = new Worker(URL
        .createObjectURL(workerBlob)
    );
    ...
    cryptoWorker.onmessage = (e) =>
    {
        ...
        if (pendingResolvers[key])
        {
            ...
            delete pendingResolvers[
                key];
        }
    };
    const workerExec = (type,
        payload) =>
    {
    ...
    const attemptHardReconnect = () =>
    {
        ...
        if (state.currentRoomId)
        {
            ...
            initRoomPresence(state
                .currentRoomId);
            setupChatChannel(state
                .currentRoomId);
        }
        else
        {
        ...
        pass) =>
    {
        ...
        if (error)
        {
            ...
            {
                window.toast(
                    `Connection lost. Retrying (${state.loginRetryCount})...`
                );
                setTimeout(() =>
                    attemptLogin(
                        email,
    ...
    $('ctx-edit').onclick = (e) =>
    {
        ...
        if (!state.contextTarget)
            return;
        state.editingMessage = state
            .contextTarget;
        $('edit-msg-input').value =
            state.contextTarget
            .text;
        window.showOverlayView(
            'edit-message');
        window.openOverlay();
        hideContextMenu();
    };
    ...
    const updateAccessSummary = (
        prefix) =>
    {
        const summaryEl = $(
            `${prefix}-access-summary`
        );
        if (!summaryEl) return;
        const count = state
            .selectedAllowedUsers
        ...
        summaryEl.innerHTML =
            `<span class="c-main">${text}</span><i data-lucide="chevron-right" class="w-16 h-16"></i>`;
    };
    ...
    window.nextCreateStep = () =>
    {
        ...
        if (state.createType ===
            'direct')
        {
            groupFields.classList
                .add('dn');
            directFields.classList
                .remove('dn');
            accessSummary.classList
                .add('dn');
            titleEl.innerText =
                "Direct Message";
            subEl.innerText =
    ...
    window.openAccessManager = async (
        prefix) =>
    {
        state
    ...
    const renderPickerSelectedUsers =
        () =>
        {
            ...
            {
                container.innerHTML =
                    `<div style="color:var(--text-mute);padding:20px 0;font-size:12px;text-align:center">No users selected.</div>`;
                $('picker-count')
                    .innerText = '0';
                return;
            }
            ...
            container.innerHTML =
                displayUsers.map(u =>
                    `<div class="picker-user-card" style="display:flex;align-items:center;padding:8px 0;border-bottom:1px solid var(--border)"><div style="width:28px;height:28px;border-radius:50%;background:#f2f2f7;overflow:hidden;margin-right:8px;display:flex;align-items:center;justify-content:center;color:var(--accent);font-weight:800;font-size:11px">${u.avatar ? `<img src="${u.avatar}">` : u.name.charAt(0)}</div><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:12px">${esc(u.name)} ${u.id === state.user.id ? '<span style="color:var(--text-mute);font-weight:500">(You)</span>' : ''}</div><div style="font-size:9px;color:var(--text-mute);font-family:monospace">${u.id}</div></div><button class="picker-remove-btn" style="background:transparent;border:none;color:var(--danger);cursor:pointer;padding:8px" onclick="window.removePickerUser('${u.id}')"><i data-lucide="x" style="width:14px;height:14px"></i></button></div>`
                ).join('');
        };
    ...
    tabChannel.onmessage = (ev) =>
    {
        ...
        )
        {
            if (state.isMasterTab)
            {
                ...
                overlay.innerHTML =
                    `<i data-lucide="log-out" style="width:48px;height:48px;margin-bottom:24px;color:var(--danger)"></i><h1 class="title">Session Moved</h1><p class="subtitle" style="margin-bottom:48px">You switched to a new tab.</p><button class="btn btn-accent" onclick="window.forceClaimMaster()">Use Here</button>`;
                overlay.classList
                    .add('active');
            }
        }
        ...
        false) =>
    {
        ...
        const isEdited = m
            .updated_at && !
            isDeleted && new Date(m
                .updated_at)
            .getTime() > new Date(m
                .created_at)
            .getTime() + 1000;
        let html = "";
        const msgDateObj = new Date(
            m.created_at);
        const currentLabel =
            getDateLabel(
                msgDateObj);
        ...
        {
            html +=
                `<div class="date-divider"><span class="date-label">${currentLabel}</span></div>`;
            state
                .lastRenderedDateLabel =
                currentLabel;
        }
        ...
        const sideClass = m
            .user_id === state.user
            ?.id ? 'me' : 'not-me';
        const safeText = isDeleted ?
            '' : esc(m.text || '')
            .replace(/"/g,
                '&quot;');
        const dataAttrs =
            `data-id="${m.id}" data-uid="${m.user_id}" data-time="${m.created_at}" data-text="${safeText}"`;
        const timeString = m.time ||
            getTimeFromDate(m
                .created_at);
        html +=
            `<div class="msg ${sideClass} ${msgClass} ${isDeleted ? 'msg-deleted' : ''} ${isOptimistic ? 'msg-pending' : ''} pop-in" ${dataAttrs}>`;
        if (isDeleted) html +=
            `${isGroupStart && !isDirect ? `<div class="msg-header"><span class="msg-user">${esc(displayName)}</span></div>` : ''}<div class="deleted-text">Message deleted</div><span class="msg-time">${timeString}</span>`;
        else
        {
            const processedText =
                processText(m.text);
            html +=
                `${isGroupStart && !isDirect ? `<div class="msg-header"><span class="msg-user">${esc(displayName)}</span></div>` : ''}<div>${processedText}</div><span class="msg-time">${timeString}${isEdited ? '<span class="edited-tag">(Edited)</span>' : ''}</span>`;
        }
        html += `</div>`;
        return html;
    };
    ...
    const loadMoreHistory =
        async () =>
        {
            ...
            container
                .insertAdjacentHTML(
                    'afterbegin',
                    '<div id="history-loader" style="text-align:center;padding:10px;font-size:11px;color:var(--text-mute)">Loading...</div>'
                );
            const
            {
                data,
    ...
    window.openRoomInfo = async () =>
    {
        ...
        const avEl = $(
            'info-avatar');
        if (display.avatar) avEl
            .innerHTML =
            `<img src="${display.avatar}">`;
        else avEl.innerText =
            display.name.charAt(
                0);
        if (room.is_direct)
    ...
    window.prepareRemovePassword =
        () =>
        {
            state.removePasswordFlag =
                true;
            $('pass-status-label')
                .innerText =
                "Will be removed";
            $('pass-status-label').style
                .color =
                "var(--danger)";
        ...
        e) =>
    {
        ...
        const room = state
            .currentRoomData;
        const isChangingPass =
            newPass.length > 0;
        const isRemovingPass =
            state
            .removePasswordFlag;
        window.setLoading(true,
            "Saving...");
    ...
    window.openVault = async (id, n,
        rawPassword, roomSalt,
        cachedData = null) =>
    {
        if (!state.user)
            return window.toast(
        ...
        if (state
            ...
            .avatar) avEl
            .innerHTML =
            `<img src="${state.currentRoomDisplay.avatar}">`;
        else avEl.innerText =
            state
            .currentRoomDisplay
            .name.charAt(0)
    ...
    window.sendMsg = async (e) =>
    {
        ...
        {
            return window.toast(
                `Message too long (max ${CONFIG.maxMessageLength} chars).`
            );
        }
        state.processingAction =
            true;
    ...
    window.handleLogin = async (e) =>
    {
        ...
        if (!navigator.onLine)
        {
            ...
            window.toast(
                "No internet and no offline account found."
            );
            window.setLoading(
                false);
            state
    ...
    window.filterRooms = () =>
    {
        ...
        if (filtered.length === 0)
        {
            ...
            filtered.map(r =>
                `<div class="room-card" onclick="window.joinAttempt('${r.id}')"><div class="chat-avatar" style="width:36px;height:36px;margin-right:10px;font-size:13px">${r.display_avatar ? `<img src="${r.display_avatar}">` : (r.display_name||'G').charAt(0)}</div><span class="room-name">${esc(r.display_name)}</span><span class="room-icon">${r.is_direct ? '<i data-lucide="user" style="width:14px;height:14px"></i>' : ''}${r.has_password ? '<i data-lucide="lock" style="width:14px;height:14px"></i>' : ''}</span></div>`
            ).join('');
    };
