...
export function initHRNchat(customConfig = {}) {
    ...
    const updateSendButtonState = () => {
        ...
        if (state.isOfflineMode) {
            btn.disabled = true; btn.style.opacity = "0.5"; input.disabled = true; input.placeholder = "You are offline.";
        } else {
            const isReady = state.isChatChannelReady;
            if (isReady && !state.isCapacityBlocked) {
                btn.disabled = false; btn.style.opacity = "1"; input.disabled = false; input.placeholder = "Message...";
            } else {
                btn.disabled = true; btn.style.opacity = "0.5"; input.disabled = true; input.placeholder = "Connecting...";
            }
        }
    };
    ...
    window.goOnline = async () => {
        ...
        if (storedEmail && storedPass) {
            ...
            } else { window.setLoading(false); window.toast("Login failed."); window.nav('scr-login'); }
        ...
        .subscribe(async (status, err) => {
            if (status === 'SUBSCRIBED') {
                ...
                state.heartbeatInterval = setInterval(async () => {
                    if (state.presenceChannel && !state.isCapacityBlocked) await state.presenceChannel.track({ user_id: myId, online_at: new Date().toISOString() });
                }, CONFIG.presenceHeartbeatMs);
            } else if (status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
                state.isPresenceSubscribed = false;
                if (!state.isOfflineMode && !state.isCapacityBlocked) { state.isReconnecting = true; setConnectionVisuals('connecting'); state.reconnectTimer = setTimeout(attemptHardReconnect, 1000); }
            }
        });
    };
    ...
    const monitorConnection = () => {
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'visible') {
                if (state.isCapacityBlocked) return;
                if (state.isOfflineMode) return;
                if (state.isMasterTab && !state.isChatChannelReady && state.currentRoomId) { attemptHardReconnect(); }
            }
        });
    };
    ...
    window.saveEditMessage = async () => {
        ...
        try {
            ...
            if (error) window.toast("Failed to edit."); else window.toast("Message updated.");
    ...
    window.nextCreateStep = () => {
        ...
        if (state.createType === 'direct') {
            groupFields.classList.add('dn'); directFields.classList.remove('dn'); accessSummary.classList.add('dn');
            titleEl.innerText = "Direct Message"; subEl.innerText = "Who are you messaging?";
        } else {
            groupFields.classList.remove('dn'); directFields.classList.add('dn'); accessSummary.classList.remove('dn');
            titleEl.innerText = "Setup"; subEl.innerText = "Details";
        }
    ...
    window.forceClaimMaster = () => {
        if (!state.isMasterTab) {
            ...
            if (state.user) { setupGlobalPresence(state.user.id); if (state.currentRoomId) attemptHardReconnect(); }
        }
    };
    ...
    window.prepareAccountPage = async () => {
        ...
        if (avatar) avPrev.innerHTML = `<img src="${avatar}">`; else avPrev.innerText = name.charAt(0);
    ...
    window.openRoomInfo = async () => {
        ...
        if (room.is_direct) {
            ...
            if (room.created_by) {
                ...
                if (!profile) { const { data } = await db.from('profiles').select('full_name').eq('id', room.created_by).single(); profile = data; if (data) state.profileCache[room.created_by] = data; }
    ...
    window.initiateDeleteRoom = () => {
        ...
        if (btn.classList.contains('active')) window.deleteRoom();
            ...
            state.deleteConfirmTimeout = setTimeout(() => { btn.classList.remove('active'); btn.innerText = "Delete Chat"; }, 3000);
        }
    };
    ...
    window.openVault = async (id, n, rawPassword, roomSalt, cachedData = null) => {
        ...
        if (!roomData) { roomData = await localDB.get('rooms', id); }
        if (!state.isOfflineMode) {
            try {
        ...
        if (!roomData) {
            if (id && n && roomSalt) { roomData = { id, name: n, salt: roomSalt, created_at: new Date().toISOString() }; }
            else { window.setLoading(false); return window.toast("Chat not found."); }
        }
        ...
        if (state.isOfflineMode) { setConnectionVisuals('offline'); }
        else { setConnectionVisuals('connecting'); await initRoomPresence(id); await setupChatChannel(id); }
        ...
        if (state.isOfflineMode) {
            ...
            if (knownUser && knownUser.metadata) {
                ...
                if (knownUser.pass_hash && knownUser.pass_hash === hashInput) {
                    ...
                    window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); state.processingAction = false; window.toast("Logged in offline."); return;
                }
            }
            window.toast("Login failed."); window.setLoading(false); state.processingAction = false; return;
        }
        ...
        if (error) {
            ...
            if (knownUser && knownUser.metadata) {
                ...
                if (knownUser.pass_hash && knownUser.pass_hash === hashInput) {
                    setAppMode(true);
                    state.user = { id: knownUser.userId, email: knownUser.email, user_metadata: knownUser.metadata };
                    window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); state.processingAction = false; window.toast("Offline mode."); return;
                }
            }
            ...
            if (state.user) setupGlobalPresence(state.user.id);
            window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); state.processingAction = false;
        }
    };
    ...
    window.handleVerify = async (e) => {
        ...
        try {
            ...
            else { window.toast("Invalid code."); window.setLoading(false); }
    ...
    window.nav = (id, direction = null) => {
        ...
        requestAnimationFrame(() => {
            ...
            if (id === 'scr-create') {
                ...
                updateStepUI('create');
                $('c-name').value = ''; $('c-target-user').value = ''; $('c-pass').value = ''; $('c-avatar').value = '';
            ...
            if (id === 'scr-register') {
                ...
                $('r-name').value = ''; $('r-email').value = ''; $('r-pass').value = ''; $('r-avatar-url').value = '';
                updateStepUI('reg');
            }
