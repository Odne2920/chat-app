...
export function initHRNchat(customConfig = {}) {
    ...
    const attemptLogin = async (email, pass) => {
        ...
        state.user = user;
        if (user) {
            const profileData = { id: user.id, full_name: user.user_metadata?.full_name, avatar_url: user.user_metadata?.avatar_url, updated_at: new Date().toISOString() };
    ...
    window.handleAvatarUpload = (event) => { const file = event.target.files[0]; if (file) { const reader = new FileReader(); reader.onload = (e) => { const img = new Image(); img.onload = () => { const canvas = document.createElement('canvas'); const size = 200; canvas.width = size; canvas.height = size; const ctx = canvas.getContext('2d'); const scale = Math.max(size / img.width, size / img.height); const x = (size - img.width * scale) / 2; const y = (size - img.height * scale) / 2; ctx.drawImage(img, x, y, img.width * scale, img.height * scale); const dataUrl = canvas.toDataURL('image/jpeg', 0.85); state.selectedAvatar = dataUrl; AVATARS.push(dataUrl); $('r-avatar-url').value = ''; updateCarouselPreview(); window.toast("Avatar uploaded."); }; img.onerror = () => window.toast("Invalid image."); img.src = e.target.result; }; reader.readAsDataURL(file); } };
    ...
    window.openAccessManager = async (prefix) => { state.currentPickerContext = prefix; if (prefix === 'edit-room' && state.selectedAllowedUsers.length === 0 && state.currentRoomData) { const ids = state.currentRoomData.allowed_users; if (ids && !ids.includes('*')) { const { data: profiles } = await db.from('profiles').select('id, full_name, avatar_url').in('id', ids); state.selectedAllowedUsers = ids.map(id => { const p = profiles?.find(pro => pro.id === id); return { id: id, name: p?.full_name || 'Unknown', avatar: p?.avatar_url }; }); } } renderPickerSelectedUsers(); window.showOverlayView('access-manager'); window.openOverlay(); state.ui.overlayCloseLocked = true; $('picker-id-input').value = ''; $('picker-id-input').focus(); };
    ...
    window.prepareAccountPage = async () => { if (!state.user) return; const { data: profile } = await db.from('profiles').select('avatar_url, full_name').eq('id', state.user.id).single(); const name = profile?.full_name || state.user.user_metadata?.full_name || "User"; const avatar = profile?.cached_avatar || profile?.avatar_url || state.user.user_metadata?.avatar_url; $('acc-page-name').innerText = name; $('acc-page-type').innerText = "Full Account"; $('acc-page-id').innerText = state.user.id; const avPrev = $('acc-page-avatar'); if (avatar) avPrev.innerHTML = `<img src="${avatar}">`; else avPrev.innerText = name.charAt(0); $('acc-email-wrapper').style.display = 'block'; $('acc-page-email').innerText = state.user.email || "Not set"; };
    ...
    window.openRoomSettings = async () => { window.closeOverlay(); if (!state.currentRoomId || !state.currentRoomData || state.currentRoomData.created_by !== state.user.id) return window.toast("Access denied."); window.setLoading(true, "Loading..."); const room = state.currentRoomData; state.currentStep.edit = 1; updateStepUI('edit'); $('edit-room-name').value = room.name; $('edit-room-visible').checked = room.is_visible; $('edit-room-pass').value = ''; const passStatusLabel = $('pass-status-label'); const removePassBtn = $('btn-remove-pass'); if (room.has_password) { passStatusLabel.innerText = "Active"; passStatusLabel.style.color = "var(--success)"; removePassBtn.style.display = 'block'; } else { passStatusLabel.innerText = "Not Set"; passStatusLabel.style.color = "var(--text-mute)"; removePassBtn.style.display = 'none'; } state.removePasswordFlag = false; state.selectedAllowedUsers = []; const ids = room.allowed_users; if (ids && !ids.includes('*')) { const { data: profiles } = await db.from('profiles').select('id, full_name, avatar_url').in('id', ids); state.selectedAllowedUsers = ids.map(id => { const p = profiles?.find(pro => pro.id === id); return { id: id, name: p?.full_name || 'Unknown', avatar: p?.avatar_url }; }); } window.showOverlayView('room-settings'); window.openOverlay(); window.setLoading(false); };
    ...
    window.openVault = async (id, n, rawPassword, roomSalt, cachedData = null) => {
        ...
        if (state.isOfflineMode) {
             ...
             if (localMessages.length > 0) {
                 let prev = null;
                 localMessages.forEach(m => { renderedHtml += renderMsg(m, prev, isDirect); prev = m; });
    ...
    const finishReg = async (temp) => { const { error } = await db.auth.signUp({ email: temp.em, password: temp.p, options: { data: { full_name: temp.n, avatar_url: temp.avatar } } }); if (error) { window.toast("Registration failed."); window.setLoading(false); } else { localStorage.setItem('hrn_auth_email', temp.em); localStorage.setItem('hrn_auth_pass', temp.p); window.nav('scr-lobby'); window.loadRooms(); window.setLoading(false); } };
    window.handleCreate = async (e) => { if (!e || !e.isTrusted) return; if (state.processingAction) return; state.processingAction = true; const isDirect = state.createType === 'direct'; let n, isVisible = true, targetUser = null; let avatarUrl = null; let rawPass = null; if (isDirect) { targetUser = $('c-target-user').value.trim(); if (!targetUser) { window.toast("User ID required."); state.processingAction = false; return; } const { data: profile, error } = await db.from('profiles').select('id, full_name, avatar_url, updated_at').eq('id', targetUser).single(); if (error || !profile) { window.toast("User not found."); state.processingAction = false; return; } await localDB.put('profiles', profile); state.profileCache[profile.id] = profile; n = "Direct Message"; isVisible = true; } else { n = $('c-name').value.trim(); const inputUrl = $('c-avatar').value.trim(); if (inputUrl) { window.setLoading(true, "Processing image..."); avatarUrl = await processAvatarUrl(inputUrl); } else { avatarUrl = null; } rawPass = $('c-pass').value; isVisible = $('c-visible').checked; if (!n) { window.toast("Name required."); state.processingAction = false; window.setLoading(false); return; } } let allowedUsers = ['*']; if (isDirect) allowedUsers = [ state.user.id, targetUser ]; else { if (state.selectedAllowedUsers.length > 0) { allowedUsers = state.selectedAllowedUsers.map(u => u.id); if (!allowedUsers.includes(state.user.id)) allowedUsers.push(state.user.id); } } window.setLoading(true, "Creating..."); const roomSalt = generateSalt(); const insertData = { name: n, avatar_url: avatarUrl, has_password: !!rawPass, is_visible: isVisible, salt: roomSalt, created_by: state.user.id, allowed_users: allowedUsers, is_direct: isDirect }; const { data, error } = await db.from('rooms').insert([ insertData ]).select(); if (error) { window.toast("Creation failed."); state.processingAction = false; window.setLoading(false); return; } if (data && data.length > 0) { const newRoom = data[0]; if (rawPass) { const accessHash = await sha256(rawPass + roomSalt); await db.rpc('set_room_password', { p_room_id: newRoom.id, p_hash: accessHash }); } await localDB.put('rooms', newRoom); state.lastCreated = newRoom; state.lastCreatedPass = rawPass; $('s-id').innerText = newRoom.id; window.nav('scr-success'); state.selectedAllowedUsers = []; } state.processingAction = false; window.setLoading(false); };
    window.submitGate = async (e) => { if (!e || !e.isTrusted) return; const inputPass = $('gate-pass').value; if (state.isOfflineMode) { window.openVault(state.pending.id, state.pending.name, inputPass, state.pending.salt, state.pending); return; } const inputHash = await sha256(inputPass + state.pending.salt); window.setLoading(true, "Verifying..."); const { data } = await db.rpc('verify_room_password', { p_room_id: state.pending.id, p_hash: inputHash }); window.setLoading(false); if (data === true) window.openVault(state.pending.id, state.pending.name, inputPass, state.pending.salt, state.pending); else window.toast("Incorrect password."); };
    window.handleLogout = async (e) => { if (!e || !e.isTrusted) return; window.setLoading(true, "Leaving..."); await cleanupChannels(); localStorage.removeItem('hrn_auth_email'); localStorage.removeItem('hrn_auth_pass'); state.user = null; setAppMode(false); state.isCapacityBlocked = false; await db.auth.signOut(); if (state.authListener) { state.authListener.unsubscribe(); state.authListener = null; } window.nav('scr-start'); window.setLoading(false); };
    ...
    const { data: { subscription } } = db.auth.onAuthStateChange(async (ev, ses) => { if (state.isOfflineMode && !ses) return; state.user = ses?.user; if (state.user && !state.isOfflineMode) setupGlobalPresence(state.user.id); const createBtn = $('icon-plus-lobby'); if (createBtn) createBtn.style.display = 'flex'; if (ev === 'SIGNED_OUT') { if (state.heartbeatInterval) clearInterval(state.heartbeatInterval); if (state.presenceChannel) state.presenceChannel.unsubscribe(); if (state.chatChannel) state.chatChannel.unsubscribe(); if (state.globalPresenceChannel) state.globalPresenceChannel.unsubscribe(); window.nav('scr-start'); } });
